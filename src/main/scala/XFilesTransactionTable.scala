...
class XFilesTransactionTable(implicit p: Parameters) extends XFilesModule()(p)
  ...
  val newRequest = cmd.fire() & funct === UInt(t_NEW_REQUEST)
  val writeData = cmd.fire() & funct === UInt(t_WRITE_DATA)
  val writeDataLast = cmd.fire() & funct === UInt(t_WRITE_DATA_LAST)
  val readDataPoll = cmd.fire() & funct === UInt(t_READ_DATA)
